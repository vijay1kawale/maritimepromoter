...
router.post('/register', trainingRegistrationValidation, async (req, res) => {
  try {
    ...
    const sheetName = process.env.SHEET_TRAINING_REGISTRATIONS || 'Training Registrations';
    const headers = [
      'Timestamp',
      'Training ID',
      'Training Title',
...
// GET /api/training/registrations - Get all training registrations (admin only)
...
// GET /api/training/registrations/:id - Get specific training registration
router.get('/registrations/:id', async (req, res) => {
  try {
    ...
    console.error('Failed to fetch training registration:', error);
    res.status(500).json({
      success: false,
...
router.put('/registrations/:id/status', async (req, res) => {
  try {
    ...
    if (status) {
      await googleSheets.updateCell(sheetName, `P${id}`, status);
    }
    if (paymentStatus) {
      await googleSheets.updateCell(sheetName, `Q${id}`, paymentStatus);
    }
