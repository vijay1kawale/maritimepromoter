...
const WorkshopRegistrationForm = ({ workshop, open, onClose }) => {
  ...
  const renderStep2 = () => (
    <div className="space-y-4">
      <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">
        Workshop Details & Expectations
      </h3>
      
      <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 mb-4">
        <h4 className="font-medium text-gray-900 dark:text-white mb-2">Selected Workshop</h4>
        <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
          <div className="flex items-center gap-2">
            <Award className="w-4 h-4 text-eco-green-4" />
            <span className="font-medium">{formData.workshopTitle}</span>
          </div>
          <div className="flex items-center gap-2">
            <Calendar className="w-4 h-4 text-eco-green-4" />
            <span>{formData.workshopDates}</span>
          </div>
          <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-eco-green-4 focus:border-transparent ${
              errors.specialRequirements ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
            } bg-white dark:bg-gray-700 text-gray-900 dark:text-white`}
      ...
      <div className="space-y-3 pt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-start gap-3">
          ...
          />
          <label className="text-sm text-gray-700 dark:text-gray-300">
            I would like to receive updates about future workshops and sustainability insights
          </label>
        </div>
      </div>
    </div>
  ...
  // Function to redirect to Razorpay payment page
  const redirectToRazorpay = async () => {
    try {
      // First submit form data to backend
      const response = await fetch('https://doctor-promoter-backend.vercel.app/api/workshop/register', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...formData,
          workshopId: workshop?.id || 'default',
          workshopTitle: workshop?.title || 'Workshop',
          workshopDates: workshop?.dates || 'TBD',
          workshopPrice: workshop?.price || 'TBD'
        }),
      });
      ...
      if (response.ok) {
        console.log('Workshop registration submitted successfully');
        ...
        // Use the actual Razorpay link from the workshop data
        const razorpayUrl = workshop?.razorpayLink || "https://rzp.io/l/default";
        ...
        // Show error message and don't proceed to payment
        console.error('Failed to submit workshop registration:', result.message);
        setErrors({ submit: result.message || 'Failed to register for workshop. Please try again.' });
      }
    } catch (error) {
